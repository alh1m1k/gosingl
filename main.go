...
func main() {
	...
	chosenDeep := app.IntOpt("deep", 1, "recursive deep")
...
func parsePackage(ctx context.Context, cfg Config) error {
	...
	for packageName := range packages {
		var files []*ast.File
		for j := range packages[packageName].Files {
			files = append(files, packages[packageName].Files[j])
		}

		for filePath := range packages[packageName].Files {
			log.Print(fmt.Sprintf("inspect file: %s", filePath))
